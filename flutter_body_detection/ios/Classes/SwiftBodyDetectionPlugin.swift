...
public class SwiftBodyDetectionPlugin: NSObject, FlutterPlugin {
    ...
    private func handleCameraFrame(sampleBuffer: CMSampleBuffer, orientation: UIImage.Orientation) {
        do {
            ...
            guard let fixedRotatedImage = UIImage.fixedOrientation(for: rotatedImage) else {
                return
            }
            ...
            guard let data = fixedRotatedImage.jpegData(compressionQuality: 60),
                  let width = fixedRotatedImage.cgImage?.width,
                  let height = fixedRotatedImage.cgImage?.height else {
                return
            }
            ...
            if self.poseDetectionEnabled {
                let pose = self.poseDetector.detectPose(image: fixedRotatedImage)
            ...
            if self.bodyMaskDetectionEnabled {
                let mask = self.selfieSegmenter.detectSegmentationMask(image: fixedRotatedImage)
